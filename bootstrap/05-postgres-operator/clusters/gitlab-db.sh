...
spec:
  ...
  privateKey:
    algorithm: ECDSA
    size: 256
  secretName: gitlab-db-tls
EOT


until $(kubectl -n cert-manager get secrets gitlab-db-tls 1>/dev/null 2>&1); do
  echo "Waiting for gitlab-db-tls"
  sleep 5
done

# enable pull based replication
kubectl -n cert-manager annotate secret gitlab-db-tls "replicator.v1.mittwald.de/replication-allowed"="true"
kubectl -n cert-manager annotate secret gitlab-db-tls "replicator.v1.mittwald.de/replication-allowed-namespaces"="gitlab"

kubectl -n gitlab get secrets gitlab-db-tls 1>/dev/null 2>&1
if [[ $? -ne 0 ]]; then
...
metadata:
  annotations:
    replicator.v1.mittwald.de/replicate-from: cert-manager/gitlab-db-tls
  name: gitlab-db-tls
  namespace: gitlab
type: kubernetes/tls
data:
...
# Custom TLS certificate. Disabled unless tls.secretName has a value.
  tls:
    secretName: "gitlab-db-tls"  # should correspond to a Kubernetes Secret resource to load
